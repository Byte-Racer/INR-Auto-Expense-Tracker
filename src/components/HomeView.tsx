...
export default function HomeView({
  ...
  const [categoryTimeRange, setCategoryTimeRange] =
    useState<CategoryTimeRange>("1M");
  ...
  const isWarning = useMemo(() => {
    ...
    if (settings.warningThresholdType === "percentage") {
      return (
        currentBalance <= (initialTotal * settings.warningThresholdValue) / 100
      );
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div
        ...
        className={cn(
          "rounded-2xl p-6 border shadow-xl relative overflow-hidden cursor-pointer transition-colors group",
          isLocked
            ? "bg-rose-950/40 border-rose-500/50 hover:bg-rose-950/60"
            : isWarning
              ? "bg-amber-950/40 border-amber-500/50 hover:bg-amber-950/60"
      ...
      >
        ...
        {isWarning && (
          <div className="mt-4 flex items-center gap-2 text-amber-500 bg-amber-500/10 p-3 rounded-lg border border-amber-500/20">
            ...
            <p className="text-sm font-medium">Warning: Balance is low!</p>
          </div>
        )}
        {isLocked && (
      ...
      <BalanceHistoryChart
        ...
        initialBalance={
          ...
          (Number(settings.initialBankBalance) || 0)
        }
      />
